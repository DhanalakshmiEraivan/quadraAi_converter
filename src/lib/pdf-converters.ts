...
async function serverConvert(file: File, operation: string, server: string, fields: Record<string, string> = {}): Promise<ConvertResult> {
  const form = new FormData();
  form.append('file', file);
  form.append('operation', operation);
  Object.entries(fields).forEach(([key, value]) => form.append(key, value));
  const response = await fetch(`${server.replace(/\/$/, '')}/convert`, { method: 'POST', body: form });
  if (!response.ok) {
    let message = `Conversion server returned ${response.status}`;
    try { const payload = await response.json(); message = payload.detail || payload.error || message; } catch { /* ignore */ }
    throw new Error(message);
  }
  const blob = await response.blob();
  const filename = response.headers.get('x-converted-filename') || file.name.replace(/\.[^.]+$/, '.pdf');
  return { blob, filename, mimeType: blob.type || 'application/pdf' };
}
...
export async function pdfToWord(file: File): Promise<ConvertResult> {
  ...
  for (let i = 1; i <= pdf.numPages; i++) {
    const page = await renderPdfPageToPng(pdf, i, 2);
    const maxWidth = 620;
    const width = Math.min(maxWidth, page.width);
    const height = Math.round(width * page.height / page.width);
    children.push(new Paragraph({
      spacing: { before: 0, after: 0, line: 240 },
      children: [new ImageRun({ data: dataUrlToUint8Array(page.dataUrl), transformation: { width, height }, type: 'png' })],
    }));
  ...
  const pageWidthTwip = Math.round(vp.width / 72 * 1440);
  const pageHeightTwip = Math.round(vp.height / 72 * 1440);
      ...
      properties: {
        page: {
          size: {
            ...
            orientation: vp.width > vp.height ? PageOrientation.LANDSCAPE : PageOrientation.PORTRAIT,
          },
  ...
  return { blob, filename: file.name.replace(/\.pdf$/i, '.docx'), mimeType: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' };
}
...
export async function pdfToPPTX(file: File): Promise<ConvertResult> {
  ...
  pptx.author = 'QuadraConverter';
  pptx.subject = 'PDF page-preserving conversion';
  ...
  for (let i = 1; i <= pdf.numPages; i++) {
    ...
    slide.addImage({ data: page.dataUrl, x, y, w, h });
  }
...
export async function pdfToExcel(file: File): Promise<ConvertResult> {
  ...
  for (let i = 1; i <= pdf.numPages; i++) {
    ...
    // Reconstruct rows from PDF text coordinates rather than flattening the
    // entire page into one cell. This gives substantially better spreadsheet
    // output for invoices, reports and table-like PDFs.
    ...
    for (const row of rows) {
      ...
      for (const cell of row.cells) {
        const gap = cell.x - currentX;
        if (gap > 35) values.push('');
        values.push(cell.text);
        currentX = cell.x + cell.width;
      }
      ws.addRow(values);
    }
    ...
    ws.columns.forEach((col) => { col.width = Math.min(40, Math.max(10, Number(col.width || 10))); });
  }
